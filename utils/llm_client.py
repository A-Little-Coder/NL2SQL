...
# LLMClient
# ──────────────────────────────────────────────────────────────────────
# [multi-session-concurrency D2 不变量] 当前所有 LLM 调用经此单一全局实例、且单条查询
# 内部串行，故请求级并发闸（src/api/concurrency_gate.py）等价 LLM 级闸（1:1）。若未来
# 引入查询内并行 LLM 调用，须在此处补 threading.Semaphore 兜底，否则请求级闸会静默
# 欠数、可能重新 429。
