# LLM Thinking Language Specification

## Purpose

让 Qwen3 等支持思考链的模型用中文进行内部推理，提升前端 SSE 展示对中文用户的友好度。
LLMClient 在请求入口统一注入中文思考指令，业务 prompt 模块零侵入；
支持通过环境变量在英文场景关闭。

注入逻辑使用 `langchain_core.messages.SystemMessage` 对象操作（migrate-llm-client-to-langchain 后从 dict 操作升级）。
...
- **THEN** Qwen3 返回的 chunk 中按 `output_version="responses/v1"` 模式解析出的 reasoning summary 文本应主要使用中文
- **AND** 业务侧通过 `stream_with_sse(...)` 推送的 SSE `llm_thinking` 事件内容应主要为中文
- **AND** 业务正文（chunk content 中的 `type="text"` block）应保持原 prompt 要求的格式不受影响
