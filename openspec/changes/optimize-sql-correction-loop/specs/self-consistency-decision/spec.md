...
### Requirement: 多个候选 SQL 执行结果一致时的决策
**Reason**: 多数投票分组逻辑被两段式评分（ScoreByData + ScoreBySQL）取代。新方案通过 LLM 评分直接选择最优候选，无需对结果哈希分组。
**Migration**: 见新 ADDED Requirement `两段式评分决策流程`。

### Requirement: 所有候选结果不同时的 LLM 决策
**Reason**: 原本的 `llm_final_decision` 被两段式评分取代。R1 评分（数据视角）已能直接选出最优候选，R2 评分（SQL 视角）仅在 R1 并列最高分=5 时触发。
**Migration**: 见新 ADDED Requirement `ScoreBySQL 第二轮 SQL 视角评分`。

### Requirement: 处理全部执行失败的情况
**Reason**: 原方案"返回错误或重新生成"被"按错误等级逐个修复"取代。
**Migration**: 见新 ADDED Requirement `全失败按错误等级逐个修复`。
