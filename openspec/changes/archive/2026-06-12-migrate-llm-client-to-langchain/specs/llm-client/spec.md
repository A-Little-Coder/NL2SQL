...
LLMClient SHALL 对外暴露 `invoke(messages, *, as_json=False, temperature=None, max_tokens=None)` 方法，作为同步阻塞调用入口；命名和参数风格 MUST 对齐 LangChain Runnable 协议。
...
LLMClient SHALL 对外暴露 `stream(messages, *, as_json=False, temperature=None, max_tokens=None)` 方法，作为流式调用入口；每次 yield 一个 `(content_chunk, reasoning_chunk)` 二元组，其中 `content_chunk` 为正文片段、`reasoning_chunk` 为思考链片段，二者均可为 `None`。
...
LLMClient SHALL 同时实现 `ainvoke` 和 `astream` 方法，作为同步 `invoke` / `stream` 的异步对应；签名和返回类型 MUST 与同步版本对齐（异步返回 `Coroutine` / `AsyncIterator`）。
...
### Requirement: LLMClient 通过 model_kwargs 启用 Qwen3 思考模式

LLMClient SHALL 在 `__init__` 时将 `enable_thinking` 配置通过 `ChatOpenAI(model_kwargs={"extra_body": {"enable_thinking": True}})` 透传到底层 API；环境变量 `LLM_ENABLE_THINKING=false` 时不透传。

#### Scenario: 启用思考模式
- **GIVEN** 环境变量 `LLM_ENABLE_THINKING=true`（默认）
- **WHEN** 构造 `LLMClient(...)`
- **THEN** `self._chat_model.model_kwargs` 应包含 `{"extra_body": {"enable_thinking": True}}`

#### Scenario: 关闭思考模式
- **GIVEN** 环境变量 `LLM_ENABLE_THINKING=false`
- **WHEN** 构造 `LLMClient(...)`
- **THEN** `self._chat_model.model_kwargs` 不应包含 `extra_body` 或不含 `enable_thinking`
...
### Requirement: LLMClient 提供 chat 方法
**Reason**: 命名与 LangChain Runnable 协议不对齐；接受 `List[Dict]` 入参丢失类型信息；与 `chat_json` / `chat_stream` 三套方法概念混淆。

**Migration**: 改用 `client.invoke(messages)`；`messages` 必须是 `List[BaseMessage]`，从 `langchain_core.messages` 导入 `SystemMessage` / `HumanMessage` 构造。

### Requirement: LLMClient 提供 chat_json 方法
**Reason**: "json" 是返回类型不应耦合在方法名；与新的 `invoke(..., as_json=True)` 等价但更冗余。

**Migration**: 改用 `client.invoke(messages, as_json=True)`；返回类型保持 `Dict[str, Any]` 不变。

### Requirement: LLMClient 提供 chat_stream 方法
**Reason**: 流式回调 `on_thinking` 参数把 SSE 推送职责耦合进 LLM 客户端，违反单一职责；新设计将累积和推送移至业务侧辅助函数。

**Migration**: 改用 `client.stream(messages)` 获取 `(content_chunk, reasoning_chunk)` 元组流；业务侧用 `stream_with_sse(...)` 自动推 SSE，用 `accumulate(...)` 仅累积。

### Requirement: LLMClient 暴露 self.client 属性
**Reason**: 直接暴露 OpenAI client 破坏封装；新设计禁止业务和测试直接访问底层客户端。

**Migration**:
- 业务侧无任何对 `self.client` 的依赖（已确认）
- 测试侧 `mock client.client.chat.completions.create` 改为 `mock client._chat_model.invoke / .stream`，但通过抽出 `_run_invoke` / `_run_stream` 私有方法作为更稳定的 mock 锚点。

## MODIFIED Requirements

### Requirement: LLMClient 自动注入中文思考指令

LLMClient SHALL 在 `invoke` / `stream` / `ainvoke` / `astream` 四个入口统一注入中文思考指令，强制 Qwen3 等支持思考链的模型用中文进行内部推理，提升前端 SSE 展示对中文用户的友好度。

#### Scenario: 自动注入到 SystemMessage
- **GIVEN** LLMClient 接收到 `invoke` / `stream` / `ainvoke` / `astream` 调用
- **WHEN** `LLM_CHINESE_THINKING` 环境变量为 true（默认值）时
- **THEN** 它应该在发送给 LLM 之前自动注入中文思考指令
- **AND** 当 messages 列表首条为 `SystemMessage` 时它应该将中文思考指令追加到该 `SystemMessage.content` 末尾（生成新对象，不修改原列表）
- **AND** 当 messages 列表首条不是 `SystemMessage` 时它应该在列表开头插入新的 `SystemMessage`（内容为中文思考指令）
...
#### Scenario: 关闭中文思考指令
- **GIVEN** LLMClient 接收到任一入口调用（`invoke` / `stream` / `ainvoke` / `astream`）
...
- **GIVEN** 业务模块的 `prompts.py` 中各 `ChatPromptTemplate` 定义
- **WHEN** 中文思考指令注入逻辑实现后
...
- **THEN** Qwen3 返回的 chunk 中 `additional_kwargs["reasoning_content"]` 应主要使用中文
...
#### Scenario: 共享注入方法
- **GIVEN** LLMClient 的四个入口方法
- **WHEN** 任一入口被调用时
- **THEN** 它应该调用同一个 `_inject_chinese_thinking(messages: List[BaseMessage])` 辅助方法
