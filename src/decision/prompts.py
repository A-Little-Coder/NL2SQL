...
【第二轮评分标准 - 严格模式】
5 完美满足：SQL 逻辑**完全覆盖**用户意图，指标、维度、筛选条件均**绝对精确**，无冗余，无缺失
4 基本满足：核心数据逻辑正确，但存在**轻微**且**不影响结论的**瑕疵（如非核心冗余字段，业务上等价的替代字段）
3 部分满足/存在小错：核心指标正确但缺失关键限定条件 / 时间范围/排序方式等次要逻辑明显错误 / SQL 存在不必要的复杂操作
2 存在严重偏差：核心指标选取错误或不完整 / 漏掉了核心业务限定条件 / SQL 结构混乱
1 严重错误：SQL 无法执行 / 字段名错误 / 查询结果与用户意图大方向完全不符
0 完全不相关：生成内容不是合法的 SQL，或完全未响应用户输入
